...
{
  vec
    .map(|v| vec_try_into(v, element_name))
    .transpose()
...
pub fn to_git_cliff_config(
  ...
  let default_changelog_config = zen_release_core::default_changelog_config(cfg.header.clone());
  let default_git_config = zen_release_core::default_git_config(pr_link);
