...
pub struct Release {
  ...
  /// When you pass this flag, `zen-release` adds the `--no-verify` flag to `cargo publish`.
  #[arg(long)]
  pub no_verify: bool,

  /// Allow dirty working directories to be packaged.
  /// When you pass this flag, `zen-release` adds the `--allow-dirty` flag to `cargo publish`.
  #[arg(long)]
...
impl Release {
  pub fn release_request(
    ...
    let git_release = if let Some(git_token) = &self.git_token {
      ...
      let release = zen_release_core::GitRelease {
...
mod tests {
  ...
  fn default_config_is_converted_to_default_release_request() {
    ...
    let expected = zen_release_core::ReleaseConfig::default();
