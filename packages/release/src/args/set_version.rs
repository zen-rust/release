...
use zen_release_core::set_version::{SetVersionRequest, SetVersionSpec, VersionChange};
...
impl SetVersion {
  ...
  fn parse_workspace_versions(self) -> anyhow::Result<BTreeMap<String, VersionChange>> {
    self
            ...
            .iter()
            .map(|v| {
                let error = Err(anyhow::anyhow!("version {v} is invalid. Format need to be `<package_name>@<version>`. E.g. `zen-release set-version serde@1.2.3`"));
