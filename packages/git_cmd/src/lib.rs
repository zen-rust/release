...
impl Repo {
  ...
  pub fn new(directory: impl AsRef<Utf8Path>) -> anyhow::Result<Self> {
    ...
    let (current_remote, current_branch) =
      Self::get_current_remote_and_branch(&directory).context("cannot determine current branch")?;
  ...
  pub fn fetch(&self, obj: &str) -> anyhow::Result<()> {
    self
      .git(&["fetch", &self.original_remote, obj])
  ...
  pub fn force_push(&self, obj: &str) -> anyhow::Result<()> {
    ...
    self
      .git(&["push", &self.original_remote, obj, "--force-with-lease"])
  ...
  pub fn add_worktree(&self, path: impl AsRef<str>, object: &str) -> anyhow::Result<()> {
    self
      .git(&["worktree", "add", "--detach", path.as_ref(), object])
  ...
  pub fn remove_worktree(&self, path: impl AsRef<str>) -> anyhow::Result<()> {
    self
      .git(&["worktree", "remove", path.as_ref()])
...
pub fn git_in_dir(dir: &Utf8Path, args: &[&str]) -> anyhow::Result<String> {
  ...
  if output.status.success() {
    ...
    let mut error = format!("error while running git in directory `{dir:?}` with args `{args:?}");
...
mod tests {
  ...
  fn inexistent_previous_commit_detected() {
    ...
    repo
      .checkout_previous_commit_at_paths(&[&file1])
      .unwrap_err();
  }
