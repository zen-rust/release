...
/// Release pull request that zen-release opened/updated.
...
/// Returns:
/// - [`ReleasePr`] if zen-release opened or updated a PR.
/// - [`None`] if zen-release didn't open any pr. This happens when all packages
///   are up-to-date.
#[instrument(skip_all)]
pub async fn release_pr(input: &ReleasePrRequest) -> anyhow::Result<Option<ReleasePr>> {
  ...
  let tmp_project_root = new_project_root(&original_project_root, tmp_project_root_parent.path())?;
...
async fn open_or_update_release_pr(
  ...
  let mut opened_release_prs = git_client
    ...
    .await
    .context("cannot get opened zen-release prs")?;

  // Check if there are opened zen-release prs with the old prefix.
  // This ensures retro-compatibility with the zen-release versions.
  // TODO: Remove this check on zen-release v0.4.0.
  ...
  // Close all zen-release prs, except one.
  let old_release_prs = opened_release_prs.iter().skip(1);
  for pr in old_release_prs {
    git_client
      .close_pr(pr.number)
      .await
      .context("cannot close old zen-release prs")?;
  }
...
async fn handle_opened_pr(
  ...
  let pr_commits = git_client
    ...
    .await
    .context("cannot get commits of zen-release pr")?;
...
async fn update_pr(
  ...
  // `opened_pr.title` still contains the version calculated by the previous zen-release run,
...
/// original branch where zen-release was run (by default it's the default branch, e.g. `main`).
fn update_pr_branch(
  commits_number: usize,
  opened_pr: &GitPr,
  repository: &Repo,
...
fn reset_branch(
  ...
  // sanity check to avoid doing bad things on non-zen-release branches
  anyhow::ensure!(
...
async fn github_force_push(
  ...
  // the GitHub API.
  // We push the zen-release changes to the temporary branch instead of the release PR branch because:
  ...
  //   because the branch is the same as the default branch. So we can't revert the latest zen-release commit and push the new one.
  // To learn more, see https://github.com/release-plz/release-plz/issues/1487
  let sha =
    github_create_release_branch(client, repository, &tmp_release_branch, commit_message).await?;
...
async fn github_create_release_branch(
  ...
  let sha = github_graphql::commit_changes(client, repository, commit_message, release_branch)
    .await
    .with_context(|| format!("failed to create commit via graphql on branch `{release_branch}`"))?;
