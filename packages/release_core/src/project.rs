...
impl Project {
  pub fn new(
    ...
    packages.retain(|p| {
      let release_metadata = release_metadata_builder
  ...
  fn render_template(
    ...
    let template =
      template.unwrap_or_else(|| default_tag_name_template(self.contains_multiple_pub_packages));
...
/// zen-release copies the user project to a temporary
...
mod tests {
  ...
  fn project_tag_template_none() {
    let local_manifest = Utf8Path::new("../../tests/fixtures/typo-in-overrides/Cargo.toml");
    let project =
      get_project(local_manifest, None, &HashSet::default(), true, None, None).expect("Should ok");
